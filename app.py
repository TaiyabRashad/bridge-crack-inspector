...
def load_models():
    ...
    if not os.path.exists("models/v2.pt"):
        with st.spinner("Loading Model V2..."):
            url = "https://github.com/TaiyabRashad/bridge-crack-inspector/raw/main/v2.pt"
            r = requests.get(url)
            with open("models/v2.pt", "wb") as f:
                f.write(r.content)
...
def draw_results(image_path, confirmed, uncertain):
    ...
    for det in confirmed:
        ...
        ax.text(box[0]+4, box[1]+18, f"CRACK  {risk}  {c:.0%}",
                ...
                bbox=dict(boxstyle="round,pad=0.3", facecolor=color, alpha=0.9, linewidth=0))
    for det in uncertain:
        ...
        ax.text(box[0]+4, box[1]+18, f"UNCERTAIN  {c:.0%}",
                ...
                bbox=dict(boxstyle="round,pad=0.3", facecolor="#7f8c8d", alpha=0.88, linewidth=0))
    ax.axis("off")
    plt.tight_layout()
    return fig
